...
(define (qs li)
  (if (empty? li)
      null
      (let* ([pivot (car li)]
             [lt (filter (lamda (x) (< x pivot)) (cdr li))]
             [gt (filter (lamda (x) (> x pivot)) (cdr li))])
...
(define (plaificar li)
	(if (empty? li)
	    ...
	    (if (list? (car li))
		   			 ...
		   			 (append (list (cdr li)) (planificar (cdr li))))))
...
(define (mapa ft li)
	(if (empty? li)
		null
		(cons (ft (clear li)) (mapa ft (cdr li)))))

; ejemplo (mapa (lambda (x) (* x x)) '(9 3 4 5 10))
; regresa '(81 9 16 25 100)
